...
if nifty is not None and len(nifty) > 1:
    n_now   = float(nifty["Close"].iloc[-1])
    n_prev  = float(nifty["Close"].iloc[-2])
    n_chg   = n_now - n_prev
    n_pct   = (n_chg/n_prev)*100
    s_now   = float(sensex["Close"].iloc[-1])
    s_prev  = float(sensex["Close"].iloc[-2])
    s_chg   = s_now - s_prev
...
market_boost = 0
if nifty is not None and len(nifty) > 1:
    nifty_1m = float(nifty["Close"].iloc[-1]) / float(nifty["Close"].iloc[-21]) - 1
